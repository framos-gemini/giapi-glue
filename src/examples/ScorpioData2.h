...
public:
    ...
    ScorpioData2 (const std::string &label) : dataLabel(label) { }

    ScorpioData2 (const std::vector<unsigned char> &binaryData) {
        deserialize(binaryData);
    }
    
    ScorpioData2 (const std::string &label, uint64_t ts) : dataLabel(label), timestamp(ts) {}
